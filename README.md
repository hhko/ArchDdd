...
## 아키텍처 이해
아키텍처 첫 번째 원칙은 **관심사의 분리**(Separation of Concerns, SoC)입니다. 관심사 기준으로 분리한 것을 아키텍처는 **레이어**(Layer)로 정의합니다.  
  
아키텍처 패턴 초창기 `Layered Architecture`부터 모든 아키텍처 패턴이 분리된 관심사를 레이어로 정의하고 있습니다. 아키텍처 패턴의 역사는 레이어 배치의 역사입니다.
